...
def test__main__float__pickle(capfd, tmp_path, repeat):
    expected_value = 1.25
...
@pytest.mark.parametrize("repeat", [1, 2])
def test__main__float__pickle_base64(capfd, repeat):
    expected_value = 1.25
    args = [
        "randog",
        "float",
...
def test__main__float__err_base64_without_pickle(capfd):
    expected_value = 1.25
    args = [
        "randog",
        "float",
    ...
    with patch.object(sys, "argv", args):
        ...
        assert out == ""
        assert (
            "randog float: error: argument --base64: not allowed without argument "
            "--pickle in this mode" in err
        )


@pytest.mark.parametrize("repeat", [1, 2])
def test__main__float__pickle_fmt(capfd, tmp_path, repeat):
    expected_value = 1.25
    args = [
        "randog",
        "float",
...
@pytest.mark.parametrize("repeat", [1, 2])
def test__main__float__pickle_list(capfd, tmp_path, repeat):
    expected_value = 1.25
    list_length = 2
    output_path = tmp_path.joinpath("out.txt")
    args = [
